 ...
 */
class OutSourcingDetailFragment :
        ...
        vm.agvtype.observe(this) {
            if (it.code == 1000) {
                ...
                if (type.QDLX == 1 && type.ZDLX == 0) { //启点自动分配
                    val dialog = DialogAgv(mContext, activity, "外协入库")
                    dialog.onOkClick { str ->
                        dialog.dismiss()
                    ...
                    val autoEndDialog = DialogAutoEnd(mContext, activity, "外协入库")
                    autoEndDialog.onOkClick {
                        autoEndDialog.dismiss()
                        val agvsub = AgvSubmitModel(
                            companyNo, accout,
                            "048", it, "", "外协入库", insmodel!!.swh, 0, ""
                        )
                        vm.agvTaskadd(agvsub)
                    }
                    autoEndDialog.show()
                } else if (type.QDLX == 0 && type.ZDLX == 0) {
                    val dialog = DialogStartToEnd(mContext, activity, "外协入库")
                    dialog.onOkClick { s, s2 ->
                        dialog.dismiss()
                        val agvsub = AgvSubmitModel(
                            companyNo, accout,
                            "048", s, s2, "外协入库", insmodel!!.swh, 0, ""
                        )
                        vm.agvTaskadd(agvsub)
                    }
                    dialog.show()
    ...
    override fun initClick() {
        v.brnOk.setOnClickListener {
            ...
            insmodel = InstorageModel(
                mlistData, username, "",
                accout, "048", companyNo, listdata!![0].ckh!!,
                "", "", "", "", "", "", ""
            )
