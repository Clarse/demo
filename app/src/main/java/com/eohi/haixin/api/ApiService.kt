...
import com.eohi.haixin.ui.main.agvmodel.AddTaskModel
import com.eohi.haixin.ui.main.agvmodel.ProductionlineModel
import com.eohi.haixin.ui.main.agvmodel.TaskResult
import com.eohi.haixin.ui.main.model.*
import com.eohi.haixin.ui.main.model.WarehouseInfo
import com.eohi.haixin.ui.work.equipment.model.*
import com.eohi.haixin.ui.work.model.*
import com.eohi.haixin.ui.work.my.model.*
import com.eohi.haixin.ui.work.process.card.LzkJdModel
import com.eohi.haixin.ui.work.tooling.model.*
import com.eohi.haixin.utils.retrofit.FatherList
import com.eohi.haixin.utils.retrofit.ServerBaseResult
...
//    fun inStorage(@Body model: InStorageModel): BaseResModel<PointResult>
    ...
    //流转卡报工信息
    @GET("api/do?method=app.scgl.lzkgx.get")
    suspend fun getLzkBg(
        @Query("gsh") gsh: String,
        @Query("lzkkh") lzkkh: String,
        @Query("gxbh") gxbh: String
    ): BaseResModel<LzkBgModel>
    ...
    suspend fun getGxAll(
        @Query("cardno") cardno: String,
        @Query("gxno") gxno: String
    ): BaseResModel<GxModel>
    ...
    suspend fun getLzkxq(
        @Query("gsh") gsh: String,
        @Query("lzkkh") lzkkh: String
    ): BaseResModel<LzkxqModel>
    ...
    //流转卡详情——接单
    @POST("api/do?method=app.scgl.lzkjd.tj")
    suspend fun jd(@Body lzkJdModel: LzkJdModel): BaseResModel<SubmitResult>
...
//    suspend fun getSubList(@Query("cardno") cardno: String): BaseResModel<LZKSubListModel>
    ...
    //刀具交换
    @GET("api/do?method=app_gzgl_djgh_get")
    suspend fun getCutterReplaceList(
        @Query("gsh") gsh: String,
        @Query("sbbh") sbbh: String
    ): BaseResModel<CutterReplaceListBean>

    @POST("api/do?method=app_gzgl_djgh_tj")
    suspend fun postCutterReplace(@Body model: CutterReplacePostBean): BaseResModel<SubmitResult>

    //合并流转卡-获取流转卡信息
    @GET("api/do?method=app.lzkhb.lzkxx.get")
    suspend fun getCardInfo(
        @Query("gsh") gsh: String,
        @Query("lzkkh") lzkkh: String
    ): BaseResModel<CardInfoModel>

    //合并流转卡- 提交
    @POST("api/do?method=app.lzkhb.lzkxx.tj")
    suspend fun submitMergeCard(@Body model: CradMergeSubModel): BaseResModel<CardSubResultModel>

    //异常管理-异常呼叫
    @GET("api/do?method=gkj_ychj_cx")
    suspend fun getAbnormalCallList(
        @Query("gsh") gsh: String,
        @Query("pageindex") pageindex: Int,
        @Query("pagesize") pagesize: Int
    ): BaseResModel<AbnormalCallModel>

    //异常呼叫-提交
    @POST("api/do?method=app.ycgl_ychj_tj")
    suspend fun subAbnormaolCall(@Body model: AbnormalCallSubmitModel): BaseResModel<SubmitResult>
